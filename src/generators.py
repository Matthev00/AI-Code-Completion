...
TOKNIZERS = {
    name: AutoTokenizer.from_pretrained(model) for name, model in MODELS.items()
}
LOADED_MODELS = {
    name: AutoModelForCausalLM.from_pretrained(model) for name, model in MODELS.items()
}
...
def main():
    ...
    for sample in dataloader:
        ...
        data.append(
            {"prefix": prefix, "middle": middle, "suffix": suffix, "outputs": outputs}
        )
